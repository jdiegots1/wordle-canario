...
import { PalabrasModal } from './components/modals/PalabrasModal'
...
function App() {
  ...
  const [isPalabrasModalOpen, setIsPalabrasModalOpen] = useState(false)
  ...
  return (
    <div className="text-black dark:text-white bg-white dark:bg-gray-900 transition-all py-8 max-w-7xl mx-auto sm:px-6 lg:px-8">
      ...
      <PalabrasModal
        isOpen={isPalabrasModalOpen}
        handleClose={() => setIsPalabrasModalOpen(false)}
        />
      
      <button
        type="button"
        className="mx-auto mt-8 flex items-center px-2.5 py-1.5 border border-transparent text-xs font-medium rounded text-indigo-700 bg-indigo-100 hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 select-none"
        onClick={() => setIsPalabrasModalOpen(true)}
      >
        {PALABRAS_GAME_MESSAGE}
      </button>
